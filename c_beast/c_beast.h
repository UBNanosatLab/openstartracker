...
#define C_BEAST_EDISON_SPEED_FACTOR 30
...
typedef const float *restrict Vec3In;
typedef float *restrict Vec3Out;
typedef const float *restrict Mat3In;
typedef float *restrict Mat3Out;
...
static int load_config(Config *c, const char *filename)
{
    FILE *f;
...
static Star make_db_star(const Config *c, float x, float y, float z, float flux, int id)
{
    Star s;
...
static Star make_img_star(const Config *c, float px, float py, float flux, int id)
{
    Star s;
    float j, k;
...
static int db_add(StarDB *db, Star s)
{
    if (db->n >= db->cap)
        return -1;
...
static int load_catalog(Work *w, StarDB *db, const char *filename, float year)
{
    FILE *f;
    ...
    memset(w->cat_keys, 0, sizeof(w->cat_keys));
    db->max_variance = w->cfg.POS_VARIANCE;
    while (fgets(line, sizeof(line), f)) {
        char *p = line;
        int nf = 0;
        while (nf < 78) {
        ...
        if (nf > 29) {
            ...
            Star s = make_db_star(&w->cfg,
                                  cos(PI * ra / 180.0) * cosdec,
                                  sin(PI * ra / 180.0) * cosdec,
                                  sin(PI * dec / 180.0),
                                  w->cfg.BASE_FLUX * powf(10.0f, -mag / 2.5f),
                                  atoi(field[1]));
            if (!key_seen(w->cat_keys, v3_key(s.v)) && db_add(db, s) < 0) {
                fclose(f);
                return -1;
            }
        }
...
static inline void star_swap(Star *a, Star *b)
{
    Star t = *a;
    *a = *b;
    *b = t;
...
static void query_init(Query *q, StarDB *db, Star *map, int *res, signed char *mask)
{
    q->map = map;
    q->n = db->n;
    q->kdresults = res;
...
static void query_reset_mask(Query *q)
{
    memset(q->kdmask, 0, (size_t)(unsigned)q->n);
}

static void query_clear_results(Query *q)
{
    while (q->kdresults_size > 0)
        q->kdmask[q->kdresults[--q->kdresults_size]] = 0;
}

static void query_sort_flux(Query *q)
{
    qsort(q->map, (size_t)q->n, sizeof(q->map[0]), cmp_flux_desc);
    q->kdsorted = 0;
}

static void query_kdsort(Query *q, const Config *c)
{
    if (!q->kdsorted) {
...
static inline void kdcheck(Query *q, int idx, const float p[static 3], float r, float min_flux)
{
    Star *s = &q->map[idx];
...
static void kdsearch(Query *q, const float p[static 3], float r, float min_flux,
...
static void query_search(Query *q, const Config *c, const float p[static 3],
...
static void query_mask_filter(Query *q, const Config *c)
{
    query_kdsort(q, c);
    for (int i = 0; i < q->n; i++) {
        int lastmask = q->kdmask[i];
        query_search(q, c, q->map[i].v,
        ...
        if (q->kdresults_size > 1 || lastmask ||
            ...
            query_clear_results(q);
        }
    }
}

static void query_mask_uniform(Query *q, const Config *c, int min_stars, signed char *keep)
{
...
static int db_from_mask(StarDB *out, Query *q)
{
    out->n = 0;
    for (int i = 0; i < q->n; i++)
        if (!q->kdmask[i] && db_add(out, q->map[i]) < 0)
            return -1;
    return 0;
}

static int db_from_results(StarDB *out, Query *q)
{
    out->n = 0;
    for (int i = 0; i < q->kdresults_size; i++)
        if (db_add(out, q->map[q->kdresults[i]]) < 0)
            return -1;
    return 0;
}
...
static int cdb_from_image(CDB *cdb, StarDB *src, Query *q,
                          Constellation *cmap, int cmap_cap, int stars_per_fov)
{
    int ns, idx = 0;
    if (db_copy(&cdb->stars, src) < 0)
        return -1;
    query_init(q, &cdb->stars, q->map, q->kdresults, q->kdmask);
    query_sort_flux(q);
    cdb->results = *q;
    cdb->map = cmap;
    ns = cdb->stars.n;
...
static int cdb_from_db(CDB *cdb, StarDB *src, Query *q,
                       Constellation *cmap, int cmap_cap, int stars_per_fov,
                       const Config *cfg, signed char *keep)
{
    int n = 0, out = 0;
    if (db_copy(&cdb->stars, src) < 0)
        return -1;
    query_init(q, &cdb->stars, q->map, q->kdresults, q->kdmask);
    query_mask_uniform(q, cfg, stars_per_fov, keep);
    for (int i = 0; i < q->n; i++) if (!q->kdmask[i]) {
        query_search(q, cfg, q->map[i].v,
...
static int fov_init(StarFov *fov, StarDB *stars, float db_max_variance, Work *w)
{
    Config *c = &w->cfg;
    fov->mask = w->fov_mask;
    ...
    for (int id = 0; id < stars->n; id++) {
        ...
        for (int x = xmin; x < xmax; x++) for (int y = ymin; y < ymax; y++) {
            ...
            if (old != -1) {
                if (fov->collision_size + 2 > MAX_COLLISION)
                    return -1;
...
static void mr_flip(MatchResult *m)
{
    int t = m->match.img_s1;
    m->match.img_s1 = m->match.img_s2;
    m->match.img_s2 = t;
}
...
static void compute_score(MatchResult *m, Work *w)
{
    Config *c = &w->cfg;
...
static int db_match(CDB *db, CDB *img, MatchResult *winner, Work *w, float *p_match)
{
    StarFov fov;
    MatchResult m;
    int nc = 0;
    ...
    *p_match = 0.0f;
    ...
    if (db->stars.n < 3 || img->stars.n < 3)
        return 0;
    if (fov_init(&fov, &img->stars, db->stars.max_variance, w) < 0)
        return -1;
    mr_init(&m, db, img, &fov, w->work_map);
    for (int n = 0; n < img->map_size; n++) {
        Constellation ic = img->map[n];
        float err = w->cfg.POS_ERR_SIGMA * w->cfg.PIXSCALE *
        ...
        for (int o = lo; o < hi; o++) {
            ...
            if (db->results.kdsorted)
                query_search(&db->results, &w->cfg, m.R[0],
                             w->cfg.MAXFOV / 2,
                             w->cfg.THRESH_FACTOR * w->cfg.IMAGE_VARIANCE);
                ...
                if (m.match.totalscore > winner->match.totalscore) {
                    if (winner->match.totalscore != -FLT_MAX) {
                        if (nc >= MAX_CANDIDATES)
                            return -1;
                        w->candidates[nc++] = winner->match;
                    }
                    mr_copy(winner, &m);
                } else {
                    if (nc >= MAX_CANDIDATES)
                        return -1;
                    w->candidates[nc++] = m.match;
                }
                mr_flip(&m);
                if (flip == 0)
                    weighted_triad(&m);
            ...
            if (db->results.kdsorted)
                query_clear_results(&db->results);
        }
    }
    if (winner->match.totalscore != -FLT_MAX) {
        double p = 1.0;
        for (int i = 0; i < nc; i++)
            if (!related(winner, &w->candidates[i]))
                p += exp((double)w->candidates[i].totalscore - winner->match.totalscore);
        *p_match = (float)(1.0 / p);
    }
    return 0;
}

static int copy_n_brightest(StarDB *dst, StarDB *src, Star *tmp, int n)
{
                   ...
                   const double *spikes, int *result, int len)
{
    ...
    if (copy_n_brightest(&bright, &img, w->tmp_stars,
        ...
        return -1;
    img_cdb.stars.v = w->tmp_stars; img_cdb.stars.cap = MAX_STARS;
    q_img.map = w->q_img_map; q_img.kdresults = w->q_img_results; q_img.kdmask = w->q_img_mask;
    if (cdb_from_image(&img_cdb, &bright, &q_img, w->img_cmap, 16,
                       w->cfg.MAX_FALSE_STARS + 2) < 0)
        return -1;
    if (db_match(global, &img_cdb, &winner, w, &p_match) < 0)
        return -1;
    if (p_match > 0.9f) {
        query_search(full_q, &w->cfg, winner.R[0],
                     w->cfg.MAXFOV / 2, w->cfg.THRESH_FACTOR * w->cfg.IMAGE_VARIANCE);
        query_search(&global->results, &w->cfg, winner.R[0],
                     w->cfg.MAXFOV / 2, w->cfg.THRESH_FACTOR * w->cfg.IMAGE_VARIANCE);
        if (db_from_results(&near, full_q) < 0)
            return -1;
        fov_cdb.stars.v = w->near_stars; fov_cdb.stars.cap = MAX_NEAR;
        q_near.map = w->q_near_map; q_near.kdresults = w->q_near_results; q_near.kdmask = w->q_near_mask;
        if (cdb_from_image(&fov_cdb, &near, &q_near, w->local_cdb_map, MAX_LOCAL_CDB,
                           global->results.kdresults_size) < 0)
            return -1;
        query_clear_results(&global->results);
        query_clear_results(full_q);

        img_full_cdb.stars.v = w->img_bright; img_full_cdb.stars.cap = MAX_STARS;
        q_img2.map = w->q_img2_map; q_img2.kdresults = w->q_img2_results; q_img2.kdmask = w->q_img2_mask;
        if (cdb_from_image(&img_full_cdb, &img, &q_img2, w->img2_cmap,
                           MAX_STARS * (MAX_STARS - 1) / 2,
                           w->cfg.MAX_FALSE_STARS + 2) < 0)
            return -1;
        if (db_match(&fov_cdb, &img_full_cdb, &fov_winner, w, &p_match) < 0)
            return -1;
        for (int i = 0; i < len; i++) {
                        ...
                        const char *catalog_path, float year)
{
    ...
    if (!w || !fov_mask || w->cfg.IMG_X <= 0 || w->cfg.IMG_Y <= 0)
        return -1;
    w->fov_mask = fov_mask;
    w->catalog_db.v = w->cat;
    w->catalog_db.n = 0;
    w->catalog_db.cap = MAX_CAT;
    w->catalog_db.max_variance = 0;
    w->filtered_db.v = w->filtered;
    w->filtered_db.n = 0;
    w->filtered_db.cap = MAX_FILTERED;
    w->filtered_db.max_variance = 0;
    if (load_catalog(w, &w->catalog_db, path, year) < 0)
        return -1;
    query_init(&w->full_q, &w->catalog_db,
               w->q_full_map, w->q_full_results, w->q_full_mask);
    query_mask_filter(&w->full_q, &w->cfg);
    query_mask_uniform(&w->full_q, &w->cfg, w->cfg.REQUIRED_STARS,
                       w->keep_full);
    if (db_from_mask(&w->filtered_db, &w->full_q) < 0)
        return -1;
    query_reset_mask(&w->full_q);
    w->global.stars.v = w->filtered;
    w->global.stars.cap = MAX_FILTERED;
    w->filtered_q.map = w->q_filtered_map;
    w->filtered_q.kdresults = w->q_filtered_results;
    w->filtered_q.kdmask = w->q_filtered_mask;
    if (cdb_from_db(&w->global, &w->filtered_db, &w->filtered_q,
                    w->cdb_map, MAX_CDB, 2 + w->cfg.DB_REDUNDANCY,
                    &w->cfg, w->keep_filtered) < 0)
        return -1;
    w->prepared = 1;
    return 0;
}
