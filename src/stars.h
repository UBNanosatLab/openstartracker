...
struct star_db {
	...
	void add_star(float x, float y, float z, float mag, int id) {
		...
		map[n].id=id;
		map[n].unreliable=0;
		map[n].photons=PHOTONS*powf(10.0,-mag/2.5);
...
struct star_query {
	...
	void kdmask_filter_catalog() {
		for (int i=0;i<stars->map_size;i++) {
			...
			kdsearch(stars->map[i].x,stars->map[i].y,stars->map[i].z,DOUBLE_STAR_PX*PIXSCALE,BRIGHT_THRESH);
