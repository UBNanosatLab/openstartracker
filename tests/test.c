                                ...
                                float *p_match)
{
    ...
    *p_match = 0.0f;
    ...
    if (beast_db_from_image(&reference_cdb, &reference, w->near_stars,
    ...
    if (beast_db_from_image(&current_cdb, &current, w->img_bright,
    ...
    if (beast_db_match(&reference_cdb, &current_cdb, &winner,
                 &w->cfg, &mw, p_match) < 0)
        return -1;
    if (*p_match > 0.0f) {
        for (int i = 0; i < current_len; i++) {
...
int main(int argc, char **argv)
{
    ...
    if (beast_load_config(&tracker->cfg, argv[arg + 1]) < 0)
        goto done_tracker;
